...
export interface VueSourceLocatorOptions {
  allowRoots?: string[];
...
const DEFAULT_ATTRIBUTE_PREFIX = "data-vsl";
const DEFAULT_ENDPOINT = "/__vue-source-locator__/open-in-editor";

export default function vueSourceLocator(
  options: VueSourceLocatorOptions = {},
): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const targetFile = path.isAbsolute(file)
          ? path.resolve(file)
          : path.resolve(allowedRoots[0] ?? process.cwd(), file);
...
function instrumentVueFile(code: string, filepath: string, attrs: LocatorAttrs) {
  ...
  const edits: Array<{ content: string; offset: number }> = [];
  ...
  walkNodes(templateAst.children, (node) => {
    ...
    const insertOffset =
      templateContentStart + element.loc.start.offset + 1 + element.tag.length;
    const absoluteLine = template.loc.start.line + element.loc.start.line - 1;
    const absoluteColumn =
      element.loc.start.line === 1
        ? template.loc.start.column + element.loc.start.column - 1
    ...
    edits.push({
      offset: insertOffset,
      content:
        ` ${attrs.fileAttr}="${escapeHtmlAttribute(normalizedFile)}"` +
        ` ${attrs.lineAttr}="${absoluteLine}"` +
        ` ${attrs.columnAttr}="${absoluteColumn}"`,
    });
  });
...
function walkNodes(nodes: Node[], visit: (node: Node) => void) {
...
function toPositiveInt(rawValue: string | null, fallback: number) {
  if (!rawValue) {
    return fallback;
  }
...
function isAllowedPath(targetFile: string, allowedRoots: string[]) {
...
function resolveEditorArgs(
  ...
  if (
    editorName === "webstorm" ||
    editorName === "webstorm64.exe" ||
    editorName === "idea" ||
    editorName === "idea64.exe"
  ) {
...
function createClientScript(options: ClientScriptOptions) {
  const triggerLabelMap: Record<ClientScriptOptions["triggerKey"], string> = {
    alt: "Alt",
    ctrl: "Ctrl",
    meta: "Meta",
    shift: "Shift",
