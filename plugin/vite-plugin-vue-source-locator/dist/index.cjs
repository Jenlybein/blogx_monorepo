  ...
  return {
    ...
    configResolved(config) {
      allowedRoots = [
        ...
        ...(options.allowRoots ?? []).map(
          (root) => import_node_path.default.resolve(config.root, root)
        )
      ];
    },
    transform(code, id) {
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        const targetFile = import_node_path.default.isAbsolute(file) ? import_node_path.default.resolve(file) : import_node_path.default.resolve(allowedRoots[0] ?? process.cwd(), file);
        ...
        if (!import_node_fs.default.existsSync(targetFile)) {
          sendJson(res, 404, {
            ok: false,
...
function instrumentVueFile(code, filepath, attrs) {
  ...
  const templateAst = (0, import_compiler_dom.parse)(templateContent, {
    comments: true
  });
  const edits = [];
  walkNodes(templateAst.children, (node) => {
    if (node.type !== import_compiler_dom.NodeTypes.ELEMENT) {
      return;
    }
    const element = node;
    ...
    if (element.props.some(
      (prop) => prop.type === import_compiler_dom.NodeTypes.ATTRIBUTE && prop.name === attrs.fileAttr
    )) {
      return;
    }
...
function normalizeFilePath(filepath, pathMode, root) {
  ...
  if (pathMode === "relative") {
    const relativePath = import_node_path.default.relative(import_node_path.default.resolve(root), absolutePath);
...
function isAllowedPath(targetFile, allowedRoots) {
  return allowedRoots.some((root) => {
    const relative = import_node_path.default.relative(import_node_path.default.resolve(root), targetFile);
...
function openInEditor(editor, file, line, column) {
  ...
  const result = (0, import_node_child_process.spawnSync)(editor, editorArgs, {
...
function resolveEditorArgs(editor, file, line, column) {
  const editorName = import_node_path.default.basename(editor).toLowerCase();
