...
- 当前推荐把“用户头像上传”视为图片上传任务链路的一个业务场景：前端先复用 `/api/images/upload-tasks` 创建上传任务，上传确认后拿到 `image_id`，再通过 `/api/users/info.avatar_image_id` 让服务端反查并写入真实头像 URL，而不是继续让业务接口直接信任用户手填 URL。
...
- 用户偏好标签现在按“ID 写入、明细读取”收口：更新资料仅提交 `like_tag_ids`，详情接口返回 `like_tag_ids + like_tag_items`。
...
- `/api/users/base` 当前已不是简单的“昵称 + 头像”接口，还会返回 `abstract`、`avatar_image_id` 以及 `view_count`、`fans_count`、`follow_count`、`article_count`、`article_visited_count` 等主页聚合统计，便于前端个人主页首屏一次拿齐展示数据。
- 最近登录页如果要支持“下线设备”，前端应优先对接 `/api/users/sessions`，而不是继续把登录事件日志 `/api/users/login/log` 当成可操作的会话数据源。
- 用户资料更新当前使用 `avatar_image_id`：前端上传成功后不再需要手填 URL，而是把 `image_id` 提交给 `/api/users/info`（管理员侧对应 `/api/users/admin/info`），后端校验图片存在且可用后再把真实 URL 写回用户资料。
...
- 文章详情接口当前同时承担作者卡片初始化和编辑态回填职责，除文章正文和互动状态外，还会返回 `author_id`、`author_avatar`、`author_avatar_image_id`、`author_name`、`author_username`、`author_abstract`、`author_created_time`，以及创作表单需要的 `category_id`、`tag_ids`、`cover_image_id`。
- 创建和更新时会先校验分类、标签，再把 Markdown 转成安全内容；摘要为空时自动从正文提取，写库后同步标签关系。
- 文章封面更新现在只接受 `cover_image_id`：前端通过图片上传任务拿到 `image_id` 后，提交给文章创建 / 更新接口，由服务端校验图片存在且可用后再回填真实 `cover` URL，不再接受用户直接传封面 URL。
...
| `/api/ai/scoring/article` | POST | 公开 / 作者 | 按 type 读取评分摘要、读取历史完整建议或重新评分并落库 |
