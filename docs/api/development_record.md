...
- 用户资料更新当前已支持 `avatar_image_id`：前端上传成功后不再需要手填 URL，而是把 `image_id` 提交给 `/api/users/info`（管理员侧对应 `/api/users/admin/info`），后端校验图片存在且可用后再把真实 URL 写回用户资料。旧 `avatar` 字段仅保留兼容，不建议新前端继续使用。
