...
3. `apps/api` 启动后把变量解析成 `conf.Config`，再交给 `app.Bootstrap(...)` 初始化基础设施；
...
这一层负责把配置、客户端和运行时服务在启动阶段一次性装配好，再以显式依赖的形式下发给 HTTP、worker 和 CLI。当前项目已经不再依赖 `global.*` 或 `Configure(...)` 这种隐式单例入口，而是以 `Bootstrap -> Infra -> Wire*` 为主干。

### 接口设计表

| 运行时对象 | 类型 | 作用 |
| ---------- | ---- | ---- |
| `app.Bootstrap(...)` | 启动装配入口 | 初始化 Snowflake、Logger、Redis、DB、ClickHouse、ES、运行时站点配置 |
...
- 启动顺序在 [main.go](E:\project\blogx_monorepo\apps\api\cmd\server\main.go) 中收口为：解析 flags -> 读取配置 -> `Bootstrap` 装配基础设施 -> `flags.Run(...)` 执行命令模式 -> 按 `role` 装配 `api / river / image-ref / cron / worker / all`。
- `Bootstrap` 只负责进程级资源初始化，不直接启动业务；真正决定“启动 HTTP 还是启动 worker”的职责放在 `WireHTTP / WireWorker`。
...
3. `app.Bootstrap(...)` 初始化 Snowflake、Logger、Redis、MySQL、ClickHouse、Elasticsearch、运行时站点配置。
4. `flags.Run(...)` 执行命令模式，例如数据库迁移、ES ensure、全量文章同步、命令行用户创建。
5. 根据 `role` 调用 `WireHTTP / WireRiver / WireImageRef / WireCron / WireWorker`。

这里采用的是“显式构造 + 依赖结构体”方案：

- `Bootstrap` 返回 `Infra`，作为进程级基础设施容器。
...
- 采集层：Fluent Bit 把结构化日志采集到 ClickHouse。
