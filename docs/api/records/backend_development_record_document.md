...
| Node.js + pnpm | 根目录 `package.json`、`apps/web/package.json`、`apps/admin/package.json` | 前端与管理端开发需要，版本只要满足 Nuxt 4 即可 |
...
| `go run ./cmd/server --es --s init` | `apps/api` | 初始化 ES 索引与 pipeline |
...
| `blog_web` | `nginx:latest` | 对外静态站点与反向代理入口 |
...
2. 部分部署配置仍然写死了默认密码，不是完全配置化。
   - 例如：
     - `deploy/docker/mysql/master/init.sql`
     - `deploy/docker/mysql/slave/init.sh`
     - `deploy/docker/fluent-bit/fluent-bit.conf`
     - `deploy/docker/redis/conf/redis.conf`
   - 如果你修改了 `.envrc` 里的数据库、Redis、ClickHouse 默认密码，必须同步检查这些文件。

3. Kafka 目前只是预留。
   - Compose 里保留了 Kafka 片段，但默认注释掉，不属于当前最小可运行环境。

4. `blog_web` 当前是 Nginx 静态站点容器。
...
| 分页 | 统一使用 `common.PageInfo`、`common.ListQuery`，复杂列表用 `common.PageIDQuery` |
...
- 列表查询优先走 `ListQuery` 复用通用分页、模糊搜索、预加载；当 join、排序、预加载同时存在时，再退回两段查询。
...
- 文章列表采用“两段查询”：先按筛选与排序拿当前页 ID，再按 ID 回表预加载作者、分类、标签，避免复杂 join 下分页失真。
...
- 置顶能力复用 `user_top_article_models`，作者级置顶和管理员级置顶可以并存，列表默认排序会把置顶结果拼回文章列表与搜索结果。
...
搜索模块目前只开放文章搜索，对外统一走 Elasticsearch。它既承担前台公共检索，也承担“猜你喜欢”“作者文章”“我自己的文章”“管理员全量搜索”等不同搜索视角。

### 接口设计表

| 接口 | 方法 | 权限 | 说明 |
| ---- | ---- | ---- | ---- |
| `/api/search/articles` | GET | 公开 / 可选登录 | 搜索文章 |

### 设计思路

- 搜索入口通过 `type` 区分场景：公开搜索、基于偏好标签的猜你喜欢、指定作者文章、我的文章、管理员文章搜索。
- 查询由 `search_service` 构建 ES DSL，排序规则、置顶逻辑、标签过滤、分类过滤都在这一层收口。
- 可选登录态的价值主要体现在个性化搜索上，例如 `type=2` 时会读取用户偏好标签参与查询重写。
- ES 负责检索与排序，文章文档由文章写接口、置顶能力和同步任务共同维护。
...
| -------- | ---- | ---- |
| `--db` | CLI | 执行数据库迁移 |
| `--es --s init` | CLI | 初始化 / 删除文章索引与 pipeline |
...
- ES 初始化和全量同步拆成两个子命令，避免“初始化索引”和“灌数据”绑定在一次操作里。
- 命令行创建用户走交互式输入，复用正式用户模型和密码加密逻辑，适合初始化管理员账号。

### 当前实现边界

- 部分命令是交互式的，不适合直接放进无人值守 CI。
...
- 启动顺序在 `cmd/server/main.go` 里非常明确：解析 flags -> 读取配置 -> 初始化雪花 ID / 日志 / Redis / DB / ClickHouse / ES -> 执行命令模式 -> 初始化运行时站点配置 -> 初始化同步相关基础设施 -> 启动定时任务和路由。
